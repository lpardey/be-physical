...
def test_get_biometrics_success(user_fixture: User, user_info_fixture: UserInfo, api_client: APIClient):
...
@pytest.mark.django_db
def test_get_biometrics_failed(client_fixture: APIClient, expected_status: status, expected_response: dict[str, Any]):
    url = reverse(f"{app_name}:{GET_BIOMETRICS_VIEW_NAME}")
...
def test_get_biometrics_permission_denied(
    ...
    expected_response = (
        ...
        else BiometricsSerializer(target_user_info).data
    )
...
def test_get_biometrics_incorrect_http_method(http_method: str, api_client_authenticated: APIClient):
    url = reverse(f"{app_name}:{GET_BIOMETRICS_VIEW_NAME}")
