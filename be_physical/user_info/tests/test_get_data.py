...
@pytest.mark.django_db
def test_get_data_unauthorized_user(api_client: APIClient):
    url = reverse(f"{app_name}:{GET_DATA_VIEW_NAME}")
...
@pytest.mark.django_db
def test_get_data_user_info_not_found(api_client_authenticated: APIClient):
    url = reverse(f"{app_name}:{GET_DATA_VIEW_NAME}")

    response: Response = api_client_authenticated.get(url)

    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == USER_INFO_NOT_FOUND_RESPONSE
